...
import './AddSupplier.css'; // Make sure to create this CSS file
...
function AddSupplier() {
  ...
  // Date validation function - prevents past dates
  ...
  const sendRequest = async () => {
    try {
      await axios.post("http://localhost:5000/suppliers", {
        ...
        price: Number(input.price)   // ✅ sending price
      });
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = (e) => {
    ...
    if (!input.name || !input.phoneno || !input.address || !input.email ||
        !input.date || !input.spicename || !input.qty || !input.price) {  // ✅ price required
...
<div className="Nav">
    ...
    <div className="addsupplier-main-content">
        
      <div className="addsupplier-header">
        <h1>Add Supplier</h1>
      ...
      <div className="addsupplier-card">
        <form onSubmit={handleSubmit}>
          <div className="addsupplier-form-group">
          ...
          <div className="addsupplier-modal-actions">
            <button
              type="submit" 
              className="addsupplier-btn" 
              disabled={dateError !== ""}
            >
              Add Supplier
            </button>
